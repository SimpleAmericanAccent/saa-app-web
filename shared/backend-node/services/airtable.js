  ...
  try {
    ...
    if (recordId) {
      ...
      const response = await fetch(`https://api.airtable.com${path}`, {
        headers: { Authorization: `Bearer ${AIRTABLE_KEY_READ_ONLY_VALUE}` },
      });
 ...
 */
export async function executeAirtableOperation({ method, path, data = null }) {
  const response = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${path}`,
    {
      method,
      headers: {
        Authorization: `Bearer ${AIRTABLE_KEY_READ_WRITE_VALUE}`,
